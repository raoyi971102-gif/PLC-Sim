# ruff: noqa: F401
"""OpcUaSim Web GUI application composition and diagnostics.
...
async def _lifespan(_application: FastAPI):
    ...
    log.info("OpcUaSim GUI started, static=%s", _STATIC_DIR)
    try:
        yield
    finally:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="OpcUaSim Web GUI 后端")
    ...
    if arguments.attach_url:
        os.environ["OPCUASIM_ATTACH_URL"] = arguments.attach_url
    if arguments.attach_csv:
        os.environ["OPCUASIM_ATTACH_CSV"] = arguments.attach_csv
