...
resolve_python() {
    local requested="${OPCUASIM_PYTHON:-}"
    local candidate=""
    ...
    if [ -n "$requested" ]; then
        ...
        if [ -z "$candidate" ]; then
            printf '[X] OPCUASIM_PYTHON does not exist or is not executable: %s\n' \
                "$requested" >&2
            return 1
        fi
        if ! python_is_311 "$candidate"; then
            printf '[X] OPCUASIM_PYTHON must be Python 3.11.x: %s (%s)\n' \
    ...
    printf '    Install Python 3.11 with its venv module, or set OPCUASIM_PYTHON.\n' >&2
